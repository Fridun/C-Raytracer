//gcc liverenderer.c -o test -lm
//gcc liverenderer.c -o livetest -lm ; ./livetest
...
array3 findcolor(float ray[], object target, collision lastcollision,float origin[], object objlist[], int objcount, int background[], light lightlist[], int lightcount, int reflectionlimit, int reflectioncount){
	...
	for (int k = 0 ; k < 3; k++){
		if (color.array[k] > 255) { color.array[k] = 255; }
	}
...
void render(uint32_t *framebuffer, int res[]){
	...
	S7.col[1] = 128; 	//green
	S7.col[2] = 0; 		//blue
	S7.shape.sphere.pos[0] = 0;
	S7.shape.sphere.pos[1] = 35;
	S7.shape.sphere.pos[2] = 90;
	S7.shape.sphere.radius = 1;
	S7.reflectivity = 0.1;
	...
	P3.col[1] = 48;
	P3.col[2] = 0;
	P3.shape.plane.normal[0] = 0.75;
	P3.shape.plane.normal[1] = 0;
	P3.shape.plane.normal[2] = -0.3;
	P3.shape.plane.offset =  -75;
	P3.reflectivity = 0.0;
	...
	P4.col[1] = 206;
	P4.col[2] = 251;
	P4.shape.plane.normal[0] = -0.75;
	P4.shape.plane.normal[1] = 0;
	P4.shape.plane.normal[2] = -0.3;
	P4.shape.plane.offset =  -75;
	P4.reflectivity = 0.0;
	...
	//int framebuffer[window1.resx*window1.resy];
	
	int reflectionlimit = 15;
	...
	for (int l = 0; l<(window1.resx*window1.resy); l++){
		...
		if (hit.hit == 1){ 
			...
			framebuffer[l] = (((int)color.array[0] << 16) + ((int)color.array[1] << 8) + (int)color.array[2]);
